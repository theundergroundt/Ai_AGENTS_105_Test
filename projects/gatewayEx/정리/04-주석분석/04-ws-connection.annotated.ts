 ...
 * import 구간 읽는 법
 * ---------------------------------------------------------------------------
 * 이 파일은 연결 1개의 lifecycle만 다루지만, 실제로는 상태 정리 포인트가 많다.
 * import를 역할별로 보면:
 * - socket/ws: 연결 메타데이터 수집
 * - auth/handshake: 인증과 timeout
 * - presence/node/session: 연결 종료 후 정리 대상
 * - message-handler: 실제 RPC 프레임 처리 위임
 */
...
function resolveSocketAddress(socket: WebSocket): {
  ...
  // ws 라이브러리 내부 raw socket에서 실제 네트워크 endpoint를 추출한다.
...
export function attachGatewayWsConnectionHandler(params: AttachGatewayWsConnectionHandlerParams) {
  ...
  // originCheckMetrics는 message handler가 origin 정책 판정을 얼마나 예외적으로 통과시켰는지 집계한다.
  ...
  wss.on("connection", (socket, upgradeReq) => {
    ...
    // connId는 이후 로그, 구독, disconnect 정리의 공통 키가 된다.
    ...
    // upgrade 단계에서 잡아둔 budget claim을 이제 connection 단계가 인계받았다는 표시다.
    /*
    ...
    // holdsPreauthBudget=true 인 동안은 아직 connect handshake가 끝나지 않았다는 뜻이다.
    ...
    const setCloseCause = (cause: string, meta?: Record<string, unknown>) => {
      // 최초 close 원인을 최대한 보존해 나중에 로그 해석이 가능하게 한다.
      if (!closeCause) {
        closeCause = cause;
      }
    ...
    // 아직 connect 요청을 받지 않았기 때문에 이 시점 client는 null 상태다.
    ...
    socket.once("close", (code, reason) => {
      ...
      // 종료 시점에도 최신 context를 다시 받아 session/node 정리를 수행한다.
    ...
    // handshake timeout은 "소켓은 열렸지만 connect를 안 보내는 클라이언트"를 정리한다.
     ...
     */
    // 실제 req/res/event 프레임 파싱은 전용 message handler에 위임한다.
    // 여기서는 연결 lifecycle과 공통 상태만 준비해 전달한다.
    attachGatewayWsMessageHandler({
      socket,
      upgradeReq,
      connId,
