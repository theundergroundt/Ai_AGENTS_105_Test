 ...
 * import 구간 읽는 법
 * ---------------------------------------------------------------------------
 * 이 파일은 path 기준 라우팅과 upgrade 진입을 처리하므로 import도 매우 넓다.
 * 하지만 역할별로 보면 다음 정도로 압축된다.
 * - node/http/https/ws: 실제 입구 객체
 * - auth/http-utils/net: 요청 인증과 클라이언트 식별
 * - hooks/plugin/control-ui/canvas: path별 분기 대상
 * - openai/models/tools/sessions: 내장 HTTP API
 */
...
// lazy import 캐시는 자주 안 쓰는 모듈을 요청이 들어올 때까지 미뤄 startup 비용을 줄인다.
...
function sendJson(res: ServerResponse, status: number, body: unknown) {
  // hook 응답처럼 간단한 JSON 응답이 많아서 공통 헬퍼로 뺀다.
...
export async function runGatewayHttpRequestStages(
  ...
  for (const stage of stages) {
    try {
      // 앞 stage가 처리 완료하면 뒤 stage는 보지 않는다. 순서가 곧 우선순위다.
      if (await stage.run()) {
        return true;
      }
    } catch (err) {
...
function buildPluginRequestStages(params: {
  ...
  // plugin-auth와 plugin-http를 나누는 이유는 "인증 판정"과 "실제 처리"를 분리하기 위해서다.
...
export function createHooksRequestHandler(
  ...
  // hooks는 외부 시스템이 때릴 수 있으므로 전용 limiter와 replay cache를 별도로 둔다.
  ...
  const resolveHookClientKey = (req: IncomingMessage): string => {
    // proxy 환경에서도 같은 발신자를 안정적으로 묶기 위해 client IP 정규화가 필요하다.
  ...
  const buildHookReplayCacheKey = (params: HookReplayScope): string | undefined => {
    // idempotency key만 믿지 않고 token/scope까지 해시해 "같은 요청" 범위를 더 정확히 잡는다.
  ...
  return async (req, res) => {
    ...
    if (!hooksConfig) {
      // hooks가 비활성화면 이 handler는 아무 일도 하지 않고 다음 stage로 넘긴다.
      return false;
    }
    ...
    if (subPath === "wake") {
      // wake는 가장 단순한 hook 타입이라 검증 후 즉시 dispatch 한다.
    ...
    if (subPath === "agent") {
      // agent hook은 sessionKey/agentId/prefix 정책까지 함께 검증해야 한다.
...
export function createGatewayHttpServer(opts: {
  ...
  // /v1/models도 호환 API 묶음에 포함되므로 chat/responses 둘 중 하나만 켜져도 활성 가능하다.
  ...
  async function handleRequest(req: IncomingMessage, res: ServerResponse) {
    ...
    try {
      // 매 요청마다 최신 config를 읽어 trusted proxies, auth 관련 정책 변경을 반영한다.
      ...
      const requestStages: GatewayHttpRequestStage[] = [
        {
          // hooks는 가장 앞에 둬야 특정 basePath가 다른 fallback에 먹히지 않는다.
          name: "hooks",
    ...
    void (async () => {
      ...
      if (canvasHost) {
        ...
        if (canvasHost.handleUpgrade(req, socket, head)) {
          // canvasHost가 자기 경로를 소비했으면 일반 gateway websocket으로 넘기지 않는다.
          return;
        }
      }
      ...
      try {
        wss.handleUpgrade(req, socket, head, (ws) => {
          ...
          // 이후 04 파일에서 connect 성공/실패 시 이 key를 사용해 budget을 해제한다.
        ...
        // upgrade 도중 예외가 나면 budget을 즉시 반납하고 소켓을 끊는다.
        socket.off("close", releaseUpgradeBudget);
