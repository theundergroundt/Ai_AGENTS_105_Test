 ...
 * import 구간 읽는 법
 * ---------------------------------------------------------------------------
 * 이 파일의 관심사는 "네트워크 객체와 프로세스 메모리 상태를 만드는 것"이다.
 * 그래서 import도 다음 묶음으로 나뉜다.
 * - node/http, ws: 실제 서버 객체
 * - canvas/hooks/plugin: HTTP 요청이 갈 수 있는 대상들
 * - broadcast/chat/dedupe: 연결 이후 메모리 상태
 * - tls/readiness/auth: 서버 입구 보호 설정
 */
...
export async function createGatewayRuntimeState(params: {
   ...
   * 를 모두 포함한다.
   */
  // startup 시점의 plugin registry를 고정해 두어 HTTP route 해석 기준이 흔들리지 않게 한다.
  pinActivePluginHttpRouteRegistry(params.pluginRegistry);
  if (params.pinChannelRegistry !== false) {
    // channel registry도 같은 이유로 함께 pin 한다.
  ...
  try {
    ...
    if (params.canvasHostEnabled) {
      try {
        // canvas host는 gateway가 정적/캔버스 자원까지 같이 서빙할 때만 필요하다.
    ...
    // broadcaster는 이 clients 집합만 알면 현재 연결된 모든 클라이언트로 이벤트를 퍼뜨릴 수 있다.
    ...
    const shouldEnforcePluginGatewayAuth = (pathContext: PluginRoutePathContext): boolean => {
      // 플러그인마다 인증 예외 경로가 다를 수 있으므로 active registry를 기준으로 매번 계산한다.
     ...
     */
    // bind mode가 auto/loopback/tailnet 같은 논리값이라서 실제 host 목록으로 푸는 단계가 필요하다.
    ...
    for (const host of bindHosts) {
      ...
      try {
        ...
        // 여러 alias가 성공적으로 bind될 수 있으므로 배열에 누적한다.
    ...
    // 대표 서버 하나는 close/상태 추적의 기준점으로 계속 들고 있는다.
    if (!httpServer) {
    ...
    // 인증 전 단계 소켓 수를 제한해 websocket 남발 공격을 완충한다.
    ...
    // registry/buffers/delta 추적값은 모두 chatRunState 내부에서 꺼내 쓰는 세부 저장소다.
    ...
    // 중간 실패 시 pin 해제를 놓치지 않도록 catch에서 정리 후 다시 예외를 올린다.
    releasePinnedPluginHttpRouteRegistry(params.pluginRegistry);
    releasePinnedPluginChannelRegistry();
    throw err;
  }
